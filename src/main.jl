function main(comm, conf; rank=0, nranks=1)
    ...
    if rank == 0
        ...
        write_block_summary("Parallelization", num_nodes=nranks, nhamster_per_node=nhamster, nthreads_kpoints=nthreads_kpoints, nthreads_bands=nthreads_bands)
    end
