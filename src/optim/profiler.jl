...
struct HamsterProfiler
    ...
    L_train_BG :: Matrix{Float64}
    L_train_system_BG :: Dict{String, Matrix{Float64}}
    L_val_BG :: Vector{Float64}
    L_val_system_BG :: Dict{String, Vector{Float64}}
    printeachbatch :: Bool
    printeachiter :: Int64
...
function print_train_status(prof, iter, batch_id; verbosity=1)
    ...
    if printit && prof.printeachbatch
        print_iteration_status(iter, Niter, batch_id, Nbatch, prof.L_train[batch_id, iter], prof.L_train_MAE[batch_id, iter], prof.L_train_BG[batch_id, iter], time_iter, time_left)
    elseif printit && !prof.printeachbatch
        print_iteration_status(iter, Niter, 0, 0, mean(prof.L_train[:, iter]), mean(prof.L_train_MAE[:, iter]), mean(prof.L_train_BG[:, iter]), time_iter, time_left)
    end
end
...
function print_val_status(prof, iter; verbosity=1)
    ...
    printit = decide_printit(1, 1, iter, prof.printeachbatch, prof.printeachiter; verbosity=verbosity)
    if printit
        println(@sprintf("   Iteration: %d / %d | Val Loss: %.4f | Val MAE: %.4f | Val_BG: %.4f | Val_ Time: %.5f s", iter, Niter, prof.L_val[iter], prof.L_val_MAE[iter], prof.L_val_BG[iter], prof.val_times[iter]))
    end
end
...
"""
function print_iteration_status(iter, Niter, batch_id, Nbatch, L_train, L_train_MAE, L_train_BG, time_iter, time_left)
    if L_train ≠ 0 && batch_id == 0
        println(@sprintf("Iteration: %d / %d | Loss: %.4f | MAE: %.4f | BG: %.4f | Time: %.5f s | ETA: %.2f s", iter, Niter, L_train, L_train_MAE, L_train_BG, time_iter, time_left))
    ...
    elseif L_train ≠ 0 && batch_id ≠ 0
        println(@sprintf("Batch %d / %d | Iteration: %d / %d | Loss: %.4f | MAE: %.4f | BG: %.4f | Time: %.5f s | ETA: %.2f s", batch_id, Nbatch, iter, Niter, L_train, L_train_MAE, L_train_BG, time_iter, time_left))
    elseif L_train == 0 && batch_id ≠ 0
...
function print_final_status(prof; verbosity=1)
    ...
    final_train_loss_BG = mean(prof.L_train_BG[:, end])
    final_val_loss_BG = prof.L_val_BG[end]
    if verbosity > 0
        println("")
        println("========================================")
        ...
        println(@sprintf("Final Train BG: %.6f", final_train_loss_BG))
        ...
        if final_val_loss ≠ 0; println(@sprintf("Final Val BG: %.6f", final_val_loss_BG)); end
    ...
    if rank == 0
        h5open(filename, "cw") do file
            ...
            if length(prof.L_train_system) > 1
                ...
                for (sylvester, L_train_system_BG) in prof.L_train_system_BG
                    g = haskey(file, sylvester) ? file[sylvester] : create_group(file, sylvester)
                    if haskey(file, sylvester)
                        g["L_train_BG"] = L_train_system_BG
                    end
                end
                for (sylvester, L_val_system_BG) in prof.L_val_system_BG
                    g = haskey(file, sylvester) ? file[sylvester] : create_group(file, sylvester)
                    if haskey(file, sylvester)
                        g["L_val_BG"] = L_val_system_BG
                    end
                end
            end
