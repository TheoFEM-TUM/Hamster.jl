...
"""
function GDOptimizer(Nε, Nk, N_eig_avg, Nε_val, Nk_val, N_eig_avg_val, N_VBM_train, N_VBM_val, conf=get_empty_config(); lr=get_lr(conf), Niter=get_niter(conf), val_weights=get_val_weights(conf))
    losses = Losses(Nε, Nk, N_eig_avg, N_VBM_train, conf, weights = false)

    val_losses = Losses(Nε_val, Nk_val, N_eig_avg_val, N_VBM_val, conf, weights = val_weights)
