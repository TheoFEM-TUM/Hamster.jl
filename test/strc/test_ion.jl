@testset "Ion" begin
    ...
    ion = Ion(Hamster.element_to_number("Na"), SVector{3}(1.0, 2.0, 3.0), SVector{3}(0.1, 0.1, 0.1))
    
    @test ion.type == Hamster.element_to_number("Na")
    ...
    types_single = Hamster.element_to_number.(["K"])
    ...
    @test ions_single[1].type == Hamster.element_to_number("K")
    @test ions_single[1].pos == SVector{3}(7.0, 8.0, 9.0)
    @test ions_single[1].dist == SVector{3}(0.3, 0.3, 0.3)

    rs_ion = rand(3, 5)
    ion_types = Hamster.element_to_number.(["H", "H", "O", "C", "Ge"])
